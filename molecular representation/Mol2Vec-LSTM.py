...
# =====================================================
# 🏗️ Three-layer LSTM model definition (unidirectional)
# =====================================================
class ThreeLayerLSTM(nn.Module):
    ...
    def __init__(
            ...
            hidden_dim_3=64,
            dropout=0.3,
            num_layers_per_block=1  # Number of layers in each LSTM block
    ):
        ...
        self.bn1 = nn.BatchNorm1d(hidden_dim_1)  # Unidirectional, so no *2 is needed

        # Second LSTM layer (unidirectional)
        self.lstm2 = nn.LSTM(
            hidden_dim_1,  # Input is the unidirectional output
            hidden_dim_2,
            num_layers=num_layers_per_block,
            batch_first=True,
            bidirectional=False,  # Changed to unidirectional
        ...
        self.fc = nn.Linear(hidden_dim_3, 1)  # Unidirectional, so no *2 is needed

    def forward(self, x, lengths=None):
        # x shape: (batch, seq_len, input_dim)

        # First LSTM layer
        lstm1_out, _ = self.lstm1(x)  # (batch, seq_len, hidden_dim_1)
        ...
        if lengths is not None:
            ...
            lstm1_last = lstm1_out[:, -1, :]  # (batch, hidden_dim_1)

        lstm1_last = self.bn1(lstm1_last)

        # Second LSTM layer
        lstm2_out, _ = self.lstm2(lstm1_out)  # (batch, seq_len, hidden_dim_2)
        ...
        if lengths is not None:
            ...
            lstm2_last = lstm2_out[:, -1, :]  # (batch, hidden_dim_2)

        lstm2_last = self.bn2(lstm2_last)

        # Third LSTM layer
        lstm3_out, _ = self.lstm3(lstm2_out)  # (batch, seq_len, hidden_dim_3)
        ...
        if lengths is not None:
            ...
            lstm3_last = lstm3_out[:, -1, :]  # (batch, hidden_dim_3)
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("🧬 Mol2Vec + Three-Layer LSTM Molecular Property Prediction")
    print("=" * 70)
    ...
    # =====================================================
    print("\n" + "=" * 70)
    print("🏗️  Building three-layer LSTM model")
    print("=" * 70)

    lstm_model = ThreeLayerLSTM(
    ...
    print(f"   LSTM layer 1:        {256} (unidirectional, output: {256})")
    print(f"   LSTM layer 2:        {128} (unidirectional, output: {128})")
    print(f"   LSTM layer 3:        {64} (unidirectional, output: {64})")
    ...
    with torch.no_grad():
        val_X = torch.FloatTensor(val_sequences).to(device)
        val_len = torch.LongTensor(val_lengths)
        predictions = lstm_model(val_X, val_len).cpu().numpy()
        actual = val_labels
    ...
    results_df.to_csv('predictions_lstm.csv', index=False)

    print(f"\n💾 Prediction results saved to: predictions_lstm.csv")
    print(f"💾 Best model saved to: best_model_lstm.pth")
