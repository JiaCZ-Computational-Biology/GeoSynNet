...
class EmbeddingCNN(nn.Module):
    """Three-layer CNN for processing embedding vectors."""

    def __init__(self, embedding_dim=384, hidden_dim=256):
        super(EmbeddingCNN, self).__init__()

        # Treat the embedding vector as a 1D sequence: (batch, 1, embedding_dim)
...
def extract_embeddings(
    ...
    print(
        f"   Number of valid SMILES: {len(smiles_list)}"
    )
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        # Use a different but deterministic seed for each epoch
...
if __name__ == "__main__":
    ...
    # File paths
    train_csv_path = (
        r"D:\pycharm\gutingle\pythonProject2\新\train_data.csv"
    )

    val_csv_path = (
        r"D:\pycharm\gutingle\pythonProject2\新\validation_data.csv"
    )
    ...
    print(
        "ChemBERTa + CNN Molecular Property Prediction"
    )
