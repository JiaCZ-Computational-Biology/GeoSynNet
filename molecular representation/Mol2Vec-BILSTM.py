...
# =====================================================
# 🧪 Mol2Vec conversion functions
# =====================================================
def mol2alt_sentence_fixed(mol, radius=1):
    """
    Convert a molecule into a "sentence" (a list of substructure identifiers)
...
def sentences2vec_sequence(sentences, model, unseen='UNK', max_len=100):
    ...
    for sentence in sentences:
        ...
        if not sentence:
            # Use a zero vector for an empty sentence
            sequences.append(np.zeros((1, model.wv.vector_size)))
            lengths.append(1)
        else:
            # Truncate or retain the original length
...
def extract_mol2vec_sequences(df, smiles_column, model, radius=1, max_len=100):
    """Extract Mol2Vec sequence embeddings from a DataFrame (for LSTM)"""
    ...
    if failed_count > 0:
        print(f"   ⚠️  Conversion failed: {failed_count} molecules")
...
class ThreeLayerBiLSTM(nn.Module):
    """Three-layer BiLSTM for processing sequence embeddings; dropout=0.3"""
    ...
    def __init__(
            ...
            hidden_dim_3=64,
            dropout=0.3,
            num_layers_per_block=1  # Number of layers in each BiLSTM block
    ):
        ...
        self.bn1 = nn.BatchNorm1d(hidden_dim_1 * 2)  # *2 because it is bidirectional

        # Second BiLSTM layer
        self.lstm2 = nn.LSTM(
            hidden_dim_1 * 2,  # Input is the bidirectional output
            hidden_dim_2,
            num_layers=num_layers_per_block,
            batch_first=True,
            bidirectional=True,
    ...
    def forward(self, x, lengths=None):
        ...
        if lengths is not None:
            # Get the last valid output according to the actual length
            batch_size = x.size(0)
            last_outputs = []
            ...
            lstm1_last = lstm1_out[:, -1, :]  # (batch, hidden_dim_1*2)
        ...
        if lengths is not None:
            ...
            lstm2_last = lstm2_out[:, -1, :]  # (batch, hidden_dim_2*2)

        lstm2_last = self.bn2(lstm2_last)

        # Third BiLSTM layer
        lstm3_out, _ = self.lstm3(lstm2_out)  # (batch, seq_len, hidden_dim_3*2)
        ...
        if lengths is not None:
            ...
            lstm3_last = lstm3_out[:, -1, :]  # (batch, hidden_dim_3*2)

        lstm3_last = self.bn3(lstm3_last)

        # Output layer
        output = self.fc(lstm3_last)  # (batch, 1)
...
if __name__ == "__main__":
    ...
    max_sequence_length = 100  # Maximum LSTM sequence length
    ...
    print("=" * 70)
    print("🧬 Mol2Vec + Three-Layer BiLSTM Molecular Property Prediction")
    print("=" * 70)
    ...
    # =====================================================
    # 5️⃣ Build and train BiLSTM model
    # =====================================================
    print("\n" + "=" * 70)
    print("🏗️  Building three-layer BiLSTM model")
    print("=" * 70)

    bilstm_model = ThreeLayerBiLSTM(
    ...
    print(f"   BiLSTM layer 1:      {256} (bidirectional, output: {256 * 2})")
    print(f"   BiLSTM layer 2:      {128} (bidirectional, output: {128 * 2})")
    print(f"   BiLSTM layer 3:      {64} (bidirectional, output: {64 * 2})")
    print(f"   Fully connected:     {64 * 2} -> 1")
    print(f"   Dropout:             0.3")
    ...
    with torch.no_grad():
        val_X = torch.FloatTensor(val_sequences).to(device)
        val_len = torch.LongTensor(val_lengths)
        predictions = bilstm_model(val_X, val_len).cpu().numpy()
        actual = val_labels
    ...
    results_df.to_csv('predictions_bilstm.csv', index=False)

    print(f"\n💾 Prediction results saved to: predictions_bilstm.csv")
    print(f"💾 Best model saved to: best_model_bilstm.pth")
